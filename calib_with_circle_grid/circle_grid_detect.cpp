...
int main( int argc, char** argv )
{
    ...
    if(argc>1)
        image_file= argv[1];
    Mat image_raw = imread(image_file, -1);
    if(image_raw.empty())
    {
    ...
    bool found = cv::findCirclesGrid(imageUndistort, circleGridSize, imageCenters, CALIB_CB_ASYMMETRIC_GRID);
    ...
    cv::drawChessboardCorners(imageUndistort, circleGridSize, imageCenters, found);
    cv::imshow(image_file, imageUndistort);
    //cv::imwrite(image_file+"_"+".png",imageUndistort);
    cv::waitKey();
#endif
